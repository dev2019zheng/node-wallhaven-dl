...
export function SearchPage() {
  ...
  const headingBadge = formState.isSubmitting
    ? { label: t("badge.searching"), tone: "info" as const }
    : searchError
    ...
    <section className="space-y-6">
      ...
      <div className="wh-dense-bento grid grid-cols-1 items-start gap-[22px] min-[1180px]:grid-cols-[minmax(0,1fr)_260px] min-[1440px]:gap-[26px]">
        <div className="min-w-0 space-y-6">
          ...
          <section aria-label={t("results")} className="space-y-4">
            <div className="flex min-h-[42px] flex-wrap items-center justify-between gap-3">
              <div className="flex min-w-0 flex-wrap items-center gap-x-4 gap-y-1">
                <h3 className="text-[14px] font-semibold text-foreground">
                  {resultSummaryLabel ?? t("empty.startTitle")}
                </h3>
                {result ? (
                  <span className="text-[13px] font-medium text-muted-foreground">
                    {t("pageOf", { current: result.meta.currentPage, last: result.meta.lastPage })}
                  </span>
                ) : null}
              </div>
              {result && result.data.length > 0 ? (
                <div className="flex flex-wrap items-center justify-end gap-3">
                  ...
                  <div className="inline-flex h-[42px] items-center rounded-[14px] border border-border bg-[var(--surface-deep)] px-4 text-[13px] font-semibold text-muted-foreground">
                    {t("perPage", { count: result.meta.perPage })}
                  </div>
                  <button
            ...
            {formState.isSubmitting ? (
              <div className="grid grid-cols-1 gap-[18px] sm:grid-cols-2 xl:grid-cols-3" aria-label={t("loadingResults")}>
            ...
            {!searchError && !formState.isSubmitting && !result ? (
              <EmptyState
                description={t("empty.startDescription")}
        ...
        <aside className="app-panel space-y-6 p-6 min-[1180px]:min-h-[600px]" aria-label={t("inspector")}>
          <div className="space-y-2">
            <h3 className="text-[20px] font-semibold leading-7 text-foreground">{t("inspector")}</h3>
